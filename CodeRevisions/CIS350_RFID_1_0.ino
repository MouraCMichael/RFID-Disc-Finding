...
void setup() {
  ...
  Serial.begin(9600);
  //Serial.begin(115200);               // Communication rate of 115200 bps (must match recieving devices bps rate)
  //while (!Serial) delay(10);        // for Leonardo/Micro/Zero
  
  Serial.println(" ");           // Confirm program is running
  Serial.println("Hello!");           // Confirm program is running
  ...
  Serial.println("Waiting for an ISO14443A Card ...");    // Shows GUI message in console
}
  ...
  if (success){
    ...
    if (uidLength == 7){
      ...
      for (uint8_t i = 0; i < 42; i++){
        ...
        Serial.print("PAGE ");        // Display the current page number
        ...
        if (success){
          nfc.PrintHexChar(data, 4);   // Dump the page data
        }
